...
def _get_api_key() -> str:
    """
    Fetch ODDS_API_KEY from environment.
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "• GitHub Actions: add it under Settings → Secrets → Actions"
        )
...
def fetch_odds_moneyline(
    ...
    if r.status_code >= 400:
        raise RuntimeError(
            f"Odds API error {r.status_code}\n"
            f"Response: {r.text}"
        )
...
def print_quota_headers(r: requests.Response) -> None:
    ...
    print(
        f"Quota headers -> remaining={remaining} "
        f"used={used} last_cost={last}"
    )
